...
function tracking(param,output_path)
...
path2segments = 'Y:\mDrives\storage4\Guillermo\segmentation_testing\output\testHalfEdge\img\img_'
...
tic
param  =  produceVertices(time,param,path2segments);
...
for time=1:(param.image_interval(2)-1)
    ...
    for track_i=1:size(inds_alive,1)
        ...
        param.tracks(end+1)=struct('t',uint16(time+1),'A',uint32(div_info(d_ind2).Area),'cent',uint16(div_info(d_ind2).Centroid+min_e_points-2),'ellipse',int16([div_info(d_ind2).MajorAxisLength div_info(d_ind2).MinorAxisLength div_info(d_ind2).Orientation]),'perim',uint32(div_info(d_ind2).Perimeter),'birth',int32(cell_i),'death',int16(0),'daughters',uint32([0 0]),'neighs',cell(1),'bounds',cell(1));
